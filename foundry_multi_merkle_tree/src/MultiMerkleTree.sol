...
contract MultiMerkleTree is MerkleTree {
    ...
    constructor(
        ...
        roots[0] = Root(zeros(_tree_levels + _group_levels - 1), 0);
    }

    function insertSubtree(bytes32 _leaf) public returns (uint32 index) {
        // insertSubtree(currTreeIndex, _leaf);
        insert(_leaf);
        insert(_leaf);
        insert(_leaf);
        insert(_leaf);
    ...
    function insertSubtree(uint32 _subtreeId, bytes32 _leaf) public {
    ...
    */
    function isKnownMerkleTree(
        bytes32 _subtreeRoot
